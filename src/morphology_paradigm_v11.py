"""Score the frozen preauthorized C2A cross-lemma benchmark v11.

v11 answer rows are evaluation-only. The target lemma/class authorization predates
the benchmark, but surface generation was disabled at freeze. This scorer measures
the current runtime without changing morphology rules or generation authority.
"""
...
BENCHMARK_PATH = Path("data/qa/morphology_paradigm_benchmark_v11.jsonl")
METADATA_PATH = Path("data/qa/morphology_paradigm_benchmark_v11.meta.json")
...
def report() -> dict:
    ...
    for surface, expected_rows in grouped.items():
        ...
        if any(
            any(_mood_matches(str(row["mood"]), candidate) for row in expected_rows)
            for candidate in candidates
        ):
            combined_mood.add(surface)
    ...
    class_entry = reviewed_class_entry("buubi")
    preauthorized = (
        class_entry is not None
        and class_entry.part_of_speech == "verb"
        and class_entry.conjugation_class == "2A"
    )
    ...
    return {
        ...
        "preauthorization": {
            "target_lemma": "buubi",
            "reviewed_class_entry_present": class_entry is not None,
            "expected_class_preauthorized": preauthorized,
            "generation_enabled_in_class_entry": (
                class_entry.generation_enabled if class_entry is not None else None
            ),
        ...
        "combined": {
            ...
            "mood_matched_unique_surface_count": len(combined_mood),
            "mood_recall": len(combined_mood) / total_surfaces,
        ...
        "holdout_integrity": {
            ...
            "runtime_rule_learning_from_v11_allowed": False,
            "pre_freeze_class_authorization_allowed": True,
        },
        "interpretation": {
            "v11_tests_preauthorized_cross_lemma_c2a_generalization": True,
            "person_tense_mood_and_class_features_required": True,
            "syncretism_is_scored": True,
            "process_discovery_is_not_the_v11_target": True,
